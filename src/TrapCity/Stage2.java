...
class frame2 extends Frame implements MouseListener,MouseMotionListener,KeyListener,Runnable
{
	
	private character Hans = new character();
	private background Back_Ground = new background(4000,700,2);
	...
	public rollingstone rs1 = new rollingstone(3200);
	...
	public frame2()
	{
		...
		if ( able_kill <=2 )
		{
			...
			new Stage2().main(null);//스테이지1을 불러온다
		}
		aaa = new Thread(this);
		setSize(995, 700);
		bgm1.start();
	...
	public void paint(Graphics g)//그림 그리기
	{
		...
		g.drawImage(Toolkit.getDefaultToolkit().getImage("gamedata\\pictures\\stage1\\stage2.jpg"), Back_Ground.getbg_x(), Back_Ground.getbg_y(), Back_Ground.getbg_w(), Back_Ground.getbg_h(), this );
		//배경 그리기
	...
	public void judge_vic()
	{
		...
		if ( able_kill == 0)
		{
			...
			JOptionPane.showMessageDialog(panel, "Stage2 Clear!!!", "Victory!!", JOptionPane.WARNING_MESSAGE);
			new Stage3().main(null);
			this.setVisible(false);
		}
	}
	...
	public void judge_trap_appear()
	{
		...
		{
			//판정 좌표 수정
			if ( Hans.char_x + Hans.char_w >= trap[i].trap_x + Back_Ground.getbg_x() - 15 && Hans.char_x <= trap[i].trap_x + Back_Ground.getbg_x() + trap[i].trap_w - 15)
			{
	...
	public void run() //본 프레임의 쓰레드. 현재 돌굴러 갈때 죽는 것만 담당한다.
	{
         ...
         if(curThread == aaa) {
           for(;;) 
           {
        	   if ( Hans.char_w + Hans.char_x >= Back_Ground.getbg_x() + 3850)
       		{
       			bgm1.stop();
       			JPanel panel = new JPanel();
       			JOptionPane.showMessageDialog(panel, "Stage2 Clear!!!", "Victory!!", JOptionPane.WARNING_MESSAGE);
