...
class frame1 extends Frame implements MouseListener,MouseMotionListener,KeyListener,Runnable
{
	...
	private SoundPlay bgm1 = new SoundPlay("gamedata\\bgm\\stage1.wav");
	...
	public void paint(Graphics g)//그림 그리기
	{
		...
		g.drawImage(Back_Ground.draw_bg(), Back_Ground.getbg_x(), Back_Ground.getbg_y(), Back_Ground.getbg_w(), Back_Ground.getbg_h(), this );
		//배경 그리기
	...
	public void judge_vic()
	{
		...
		if ( able_kill == 0 )
		{
			...
			JOptionPane.showMessageDialog(panel, "Stage1 Clear!!!", "Victory!!", JOptionPane.WARNING_MESSAGE);
	...
	public void judge_trap_appear()
	{
		...
		{
			//판정 좌표 수정
			if ( Hans.char_x + Hans.char_w >= trap[i].trap_x + Back_Ground.getbg_x() - 20 && Hans.char_x <= trap[i].trap_x + Back_Ground.getbg_x() + trap[i].trap_w - 20)
			{
